...
class Rest {
    ...
    public function get_directory( WP_REST_Request $request ): array {
        ...
        foreach( $directory_list as $url => $entry ) {
            ...
            foreach( $entry['dirs'] as $path => $sub ) {
                ...
                // create URL for this directory.
                $subdirectory = $path;

                // bail if URL is already loaded.
                if( isset( $directory_list[$subdirectory] ) ) {
                    continue;
                }
     ...
     * @param array $directory_list The tree.
     *
     ...
     * Build the tree.
     *
     * @param array $directories The list of all directories.
    ...
    private function build_tree( array $directory_list, string $parent_url = '' ) {
        ...
        foreach( $directory_list as $url => $entry ) {
            ...
            foreach( $entry['dirs'] as $path => $subdir ) {
                ...
                // add the children.
                if( $children ) {
                    $entry['dirs'][$path] = array_shift( $children );
                }
