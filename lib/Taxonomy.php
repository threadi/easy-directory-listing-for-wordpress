...
class Taxonomy {
	...
	public function init(): void {
		// register taxonomy for the credential archive.
	...
	/**
	 * Return the name of our taxonomy.
	 *
	 * @return string
	 */
	...
	/**
	 * Set actions for each entry in the table.
	 *
	...
	/**
	 * Add a term in this taxonomy. Checks also if a term with the given directory does not exist.
	 *
	 * @param string              $type The listing type name.
	 * @param string              $directory The directory.
	 * @param array<string,mixed> $fields The fields used by the directory listing.
	 *
	 * @return int
	 */
	public function add( string $type, string $directory, array $fields ): int {
		// create a slug with type and directory.
		$slug = md5( $type . $directory );

		// get term with given directory to prevent double entries.
		$term = get_term_by( 'slug', $slug, $this->get_name() );

		// bail if term already does exist.
		...
		add_term_meta( $term_id, 'fields', Crypt::get_instance()->encrypt( (string) wp_json_encode( $fields ) ) );
	...
	public function delete( int $term_id ): bool {
		...
		// delete term.
		$delete_result = wp_delete_term( $term_id, $this->get_name() );

		// bail on any error.
		if ( ! is_bool( $delete_result ) ) {
			return false;
		}

		// return true if we could delete the term.
		return true;
	}
		...
		if ( empty( $fields ) && ( ! $login || ! $password || ! $api_key ) ) {
			...
			switch ( get_term_meta( $term_id, 'type', true ) ) {
				case 'google-drive':
					$fields = array(
						'access_token' => array(
							'value' => Crypt::get_instance()->encrypt( $login ),
						),
						'folder_id'    => array(
							'value' => Crypt::get_instance()->encrypt( $password ),
						),
					);
					break;
				case 'ftp':
					$fields = array(
						'server'   => array(
							'value' => Crypt::get_instance()->encrypt( $api_key ),
						),
						'login'    => array(
							'value' => Crypt::get_instance()->encrypt( $login ),
						),
			...
			update_term_meta( $term_id, 'fields', $fields );
		}
