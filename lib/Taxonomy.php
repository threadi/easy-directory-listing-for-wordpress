...
class Taxonomy {
     ...
     * @param array $fields The fields used by the directory listing.
     *
     * @return int
     */
    public function add( string $type, string $directory, array $fields ): int {
        ...
        add_term_meta( $term_id, 'fields', Crypt::get_instance()->encrypt( wp_json_encode( $fields ) ) );
    ...
    public function get_entry( int $term_id ): array {
        ...
        $password = Crypt::get_instance()->decrypt( get_term_meta( $term_id, 'password', true ), true );
        $api_key = Crypt::get_instance()->decrypt( get_term_meta( $term_id, 'api_key', true ), true );
    ...
    public function set_fields( WP_Term $term ): void {
        // get the values.
        $type     = get_term_meta( $term->term_id, 'type', true );
        $fields    = Crypt::get_instance()->decrypt( get_term_meta( $term->term_id, 'fields', true ) );
